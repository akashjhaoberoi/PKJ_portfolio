...
export function Software({ software }: SoftwareProps) {
  ...
  const getSoftwareIcon = (name: string) => {
    ...
    if (lower.includes('empire')) return Database;
    if (lower.includes('tally')) return Server;
    if (lower.includes('tamanna')) return Layers;
    if (lower.includes('tripta')) return Database;
    return AppWindow;
  };
  ...
  const getSoftwareLogo = (name: string) => {
    ...
    if (lower.includes('empire')) {
      return {
        src: '/images/software/empire-logo.png',
        alt: 'Empire ERP',
        className: 'h-8 w-8 object-contain rounded-md',
        containerClassName: 'h-11 w-11 px-1',
      };
    }
    if (lower.includes('tamanna')) {
      return {
        src: '/images/software/tamanna-logo.png',
        alt: 'Tamanna Accounting',
        className: 'h-8 w-8 object-contain rounded-md',
        containerClassName: 'h-11 w-11 px-1',
      };
    }
    ...
    if (lower.includes('tally')) {
      return {
        src: '/images/software/tally-prime-logo.png',
        alt: 'Tally Prime',
        className: 'h-8 w-8 object-contain rounded-md',
        containerClassName: 'h-11 w-11 px-1',
      };
    }
    if (lower.includes('tripta')) {
      return {
        src: '/images/software/tripta-erp-logo.png',
        alt: 'Tripta ERP',
        className: 'h-8 w-8 object-contain rounded-md',
        containerClassName: 'h-11 w-11 px-1',
      };
