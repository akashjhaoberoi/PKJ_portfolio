...
export function CVModal({ isOpen, onClose, data }: CVModalProps) {
  ...
  const handleCopyText = () => {
    const profile: Partial<ProfileInfo> = data?.profile || {
      fullName: 'Prakash Kumar Jha',
      professionalTitle: 'Accountant | M.COM Accounting & Finance',
      location: 'Surat, Gujarat',
      email: 'prakash.financeworks@gmail.com',
      primaryPhone: '9162381655',
      altPhone: '9162381655',
      drivingLicense: 'Valid Indian Driving License',
      summary: '',
    };
    ...
    let text = `${(profile.fullName || 'Prakash Kumar Jha').toUpperCase()}\n`;
    text += `${profile.professionalTitle || 'Accountant'}\n`;
    text += `Location: ${profile.location || 'Surat, Gujarat'}\n`;
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-[#0F2747] dark:text-white">
                Prakash Kumar Jha — Professional CV
              </h3>
